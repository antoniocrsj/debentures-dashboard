// lupa-mercado.mjs -- inspeciona a varredura MERCADO x DIRETA nos ultimos N pregoes.
...
import { MIN_VOL_DOBRADO, FEE_LO, FEE_HI, gruposDoDia, durationDe, varrer, agregarMercado,
...
const dias = fs.readdirSync(BDIDIR).filter(f => /^DEB_\d{4}-\d{2}-\d{2}\.csv\.gz$/.test(f)).map(f => f.slice(4, 14)).sort().slice(-N)
console.log(`\n=== LUPA MERCADO x DIRETA -- ultimos ${dias.length} pregoes (${dias[0]} .. ${dias[dias.length - 1]}) ===`)

if (!ALVO) {
  const linhas = []
  for (const dia of dias) {
    const porAtivo = gruposDoDia(BDIDIR, dia); let vd = 0, vm = 0, nAt = 0
    for (const [ativo, grupos] of porAtivo) {
      const volDobrado = grupos.reduce((s, g) => s + g.vol, 0); if (volDobrado < MIN_VOL_DOBRADO) continue
      const dd = durationDe(ativo, dia, anbima, vencDeb); if (!dd) continue
      vd += volDobrado; nAt++                              // denominador: TODO ativo relevante
    ...
    console.log(`  ${dia}: ${String(nAt).padStart(3)} ativos >=10MM | dobrado ${brl(vd).padStart(18)} | MERCADO ${brl(vm).padStart(16)} (${(vm / (vd / 2) * 100).toFixed(0)}% do econ)`)
  }
  ...
  console.log(`\n  (${linhas.length} ativos-dia relevantes. Zoom: node tools/lupa-mercado.mjs ${N} TICKER)`)
} else {
  for (const dia of dias) {
    const grupos = gruposDoDia(BDIDIR, dia).get(ALVO)
    ...
    if (volDobrado < MIN_VOL_DOBRADO) { console.log(`    (abaixo de 10MM -> irrelevante)`); continue }
    if (!dd) { console.log(`    (sem duration/prazo -> nao classificavel)`); continue }
    console.log(`    banda de fee: ${(FEE_LO * dd.D * 1e4).toFixed(1)}..${(FEE_HI * dd.D * 1e4).toFixed(1)} bps de preco`)
