...
export default function MonthBars({ rows, max, selMes, onPick, fmtVal, fmtLabel, ariaLabel, compacto = false }) {
  ...
  return (
    <div className="venc-chart" role="group" aria-label={ariaLabel}>
        ...
        {rows.map(m => {
          ...
          return (
            <button key={m.mes} type="button"
              ...
              {/* Total acima da barra; juros/amort vão DENTRO dos segmentos (inclusive
                  no compacto/Técnico), quando o segmento é alto o bastante (showJ/showA). */}
      ...
      {/* Legenda ABAIXO do eixo de meses, como no grafico da Captacao (Recharts
          poe a legenda embaixo). Antes ela ficava entre as barras e o eixo. */}
      {compacto && (
