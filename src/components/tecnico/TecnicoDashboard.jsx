...
export default function TecnicoDashboard({ agenda12m, blc, plByGestor, assets, emissoesAnbima, desktop, onAssetInfo, anbimaRef, recompraRef, corte, onCorte, corteDisponivel, pctPorCnpj }) {
  ...
  const { rows: enquadRows, serie: enquadSerie, serieGestora: enquadSerieG } = useEnquadramento12431(mostra('enquadramento'))
      ...
      {!error && rows.length > 0 && (
        ...
        {mostra('enquadramento') && (
          <div className="tecnico-enq-row">
            <Enquadramento12431 rows={enquadRows} serie={enquadSerie} serieGestora={enquadSerieG} gestor={gestorSel} />
          </div>
        )}
        </>
      )}
